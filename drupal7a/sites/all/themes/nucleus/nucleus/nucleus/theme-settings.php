...
function nucleus_form_system_theme_settings_submit($form, &$form_state) {
  ...
  foreach ($blocks as $block) {
    ...
    if (($block->region != $inputs['region_block_hidden_' . $block_key] ||
    $block->weight != $inputs['block_weight_hidden_' . $block_key])
    && !($inputs['region_block_hidden_' . $block_key] == BLOCK_REGION_NONE && $block->module == 'system' && $block->delta == 'main')) {
      $updated_list[] = array(
        'bid' => $block->bid,
        'region' => $inputs['region_block_hidden_' . $block_key],
        'weight' => $inputs['block_weight_hidden_' . $block_key],
      );
    }
  }
  ...
  if ($layout_width_selector != 'custom') {
    nucleus_auto_generate_fixed_grids($grid, $layout_width_selector);
  }
  else {
    ...
    if (!$width) {
      ...
      nucleus_auto_generate_fixed_grids($grid, $width);
    }
    elseif ($width > 100) {
      while ($width % $grid != 0) {
        $width --;
